...
一套面向 60 秒以上口播科普视频的 Codex Skill 与可执行门禁，没有 240 秒硬上限：先锁定真实口播，再用固定纸艺剧场、独立 AI 素材和确定性后期，让画面逐件登台并与每一句解释严格对齐。V1.2 新增语义动作、真实插槽/出口、同源分层和静音可读四组结构化门禁。更长的视频继续按自然章节扩展，同时按总时长增加审核帧和高密区抽查数量。
